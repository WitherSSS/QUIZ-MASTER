...
import { 
  ...
  Copy, Check, Send, ThumbsUp, ThumbsDown, History
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

// --- 常量与默认设置 ---
const APP_ID = 'smart-quiz-v7';
...
export default function App() {
  ...
  // 核心数据结构：stats 存储次数和最后一次的选择
  const [stats, setStats] = useState({}); // { [bankId]: { [qId]: { correct: 0, total: 0, lastChoice: null } } }
  const [progress, setProgress] = useState({}); // { [bankId]: lastIndex }
      ...
      return {
        ...
        [currentBank.id]: {
          ...
          [q.id]: {
            ...
            total: qStat.total + 1,
            lastChoice: choice // 记录最后一次选择，用于变红显示
          }
        }
      };
    });
      ...
      {resumeModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-sm bg-white dark:bg-gray-900 rounded-[2.5rem] p-8 shadow-2xl text-center">
            ...
            <h3 className="text-xl font-bold dark:text-white mb-2">恢复进度？</h3>
            ...
            <div className="space-y-3">
              <button onClick={() => startQuiz(resumeModal.bank, resumeModal.mode, resumeModal.index)} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold active:scale-95 transition-all">继续练习</button>
              <button onClick={() => startQuiz(resumeModal.bank, resumeModal.mode, 0)} className="w-full py-4 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-2xl font-bold active:scale-95 transition-all">从头开始</button>
            </div>
          </motion.div>
        </div>
      )}
    ...
    <div className={`fixed inset-y-0 right-0 w-80 bg-white dark:bg-gray-900 shadow-2xl z-[150] transform transition-transform duration-300 ease-in-out ${isDrawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
      <div className="p-6 h-full flex flex-col">
        ...
        <div className="flex-1 space-y-8 overflow-y-auto pr-1 no-scrollbar">
          <section>
            <label className="text-xs font-bold text-gray-400 uppercase mb-3 block tracking-widest">主题</label>
          ...
          <section className="space-y-4">
            <label className="text-xs font-bold text-gray-400 uppercase block tracking-widest">偏好</label>
            {[
              { label: '答对自动跳转', key: 'autoNext' },
              { label: '未做优先', key: 'unvisitedFirst' },
          ...
          <section>
            <label className="text-xs font-bold text-gray-400 mb-3 block flex justify-between tracking-widest">字体 <span>{settings.fontSize}px</span></label>
          ...
          <section className="space-y-3 pt-4 border-t dark:border-gray-800">
            <button onClick={() => { setStats({}); setProgress({}); showToast("刷题数据已重置"); }} className="w-full py-3 bg-red-50 dark:bg-red-900/10 text-red-600 border border-red-100 dark:border-red-900/30 rounded-xl font-bold flex items-center justify-center gap-2">
              <Eraser className="w-4 h-4"/> 清除做题记录
            </button>
<button onClick={() => { if(window.confirm('确定清除所有题库和设置吗？')){ localStorage.clear(); window.location.reload(); } }} className="w-full py-3 bg-gray-100 dark:bg-gray-800 text-gray-500 rounded-xl font-bold">清除所有记录并重置</button>
            <button onClick={() => { const d = { banks, stats, settings }; const b = new Blob([JSON.stringify(d)], {type:'application/json'}); const u = URL.createObjectURL(b); const a = document.createElement('a'); a.href=u; a.download='quiz_backup.json'; a.click(); }} className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all active:scale-95">
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-6 flex flex-col items-center">
      <div className="w-full max-w-2xl">
        <header className="flex justify-between items-center py-8">
          ...
          <button onClick={() => setIsDrawerOpen(true)} className="p-2 bg-white dark:bg-gray-900 rounded-xl shadow-sm border dark:border-gray-800 active:scale-90"><Settings className="dark:text-gray-400"/></button>
        </header>
        <div className="grid grid-cols-1 gap-6 pb-12">
          <div className="bg-white dark:bg-gray-900 p-8 rounded-[2.5rem] shadow-sm border dark:border-gray-800">
            <h2 className="font-bold text-gray-400 text-xs uppercase tracking-widest mb-4">导入题库</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col justify-center gap-2">
                <div className="relative">
                  ...
                  <input type="text" placeholder="URL 加载..." className="w-full pl-8 pr-4 py-3 bg-gray-50 dark:bg-gray-800 border-none rounded-xl text-xs dark:text-white outline-none focus:ring-1 ring-blue-500" onKeyDown={e => {
                ...
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <div className="flex gap-2 mt-1">
                      ...
                      {progress[bank.id] > 0 && <span className="text-[10px] bg-blue-100 dark:bg-blue-900/30 text-blue-600 px-2 py-0.5 rounded-full">进度 {progress[bank.id] + 1}/{bank.count}</span>}
                    </div>
                  </div>
...
<div className="flex items-center gap-2">
            ...
            <button onClick={() => setIsSheetOpen(true)} className="p-2 bg-gray-100 dark:bg-gray-800 rounded-lg"><Menu className="w-5 h-5 dark:text-white"/></button>
            <button onClick={() => setIsDrawerOpen(true)} className="p-2 bg-gray-100 dark:bg-gray-800 rounded-lg"><Settings className="w-5 h-5 dark:text-white"/></button>
          </div>
        </header>
        ...
        <div className="flex-1 relative overflow-hidden flex flex-col">
          <AnimatePresence mode="wait" initial={false} custom={direction}>
            <motion.div
              ...
              initial="enter" animate="center" exit="exit" transition={{ type: "spring", damping: 30, stiffness: 300 }}
              drag="x" dragConstraints={{ left: 0, right: 0 }} dragElastic={0.2}
              onDragEnd={(e, i) => { if(i.offset.x > 50) navigate(-1); else if(i.offset.x < -50) navigate(1); }}
              className="absolute inset-0 p-6 flex flex-col h-full overflow-y-auto pb-32 no-scrollbar touch-pan-y"
            >
              <div className="max-w-2xl mx-auto w-full">
                <motion.div whileTap={{ scale: 0.98 }} onClick={() => copyQuestion(q)} className="font-bold text-gray-800 dark:text-gray-100 leading-relaxed mb-8 cursor-pointer p-2 -m-2 rounded-2xl transition-all relative group" style={{ fontSize: `${settings.fontSize}px` }}>
                  ...
                  {q.title}
                  <div className="absolute top-0 right-0 opacity-0 group-hover:opacity-100"><Copy className="w-3 h-3 text-gray-300"/></div>
                </motion.div>

                <div className="space-y-3">
                  {type === 'short' ? (
                    <div className="space-y-4">
                      <textarea value={shortAnswerText} onChange={e => setShortAnswerText(e.target.value)} disabled={showResult} placeholder="在此输入简答..." className="w-full h-44 p-5 bg-gray-50 dark:bg-gray-900 border-2 border-gray-100 dark:border-gray-800 rounded-3xl text-sm focus:ring-2 ring-blue-500 outline-none transition-all dark:text-white resize-none" />
                      {!showResult && !showShortResult && (
                        <button onClick={() => setShowShortResult(true)} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold flex items-center justify-center gap-2 active:scale-95">查看答案解析 <Send className="w-4 h-4"/></button>
                      )}
                    </div>
                  ) : (
...
{(showResult || showShortResult) && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-8 p-6 bg-blue-50/50 dark:bg-blue-900/10 rounded-[2rem] border border-blue-100 dark:border-blue-900/30">
                    ...
                    {type === 'short' && !answered && (
                      <div className="mt-6 flex gap-3">
                        <button onClick={() => handleAnswer(null, true)} className="flex-1 py-4 bg-green-500 text-white rounded-2xl font-bold active:scale-95"><ThumbsUp className="w-4 h-4"/> 我答对了</button>
                        <button onClick={() => handleAnswer(null, false)} className="flex-1 py-4 bg-red-500 text-white rounded-2xl font-bold active:scale-95"><ThumbsDown className="w-4 h-4"/> 我答错了</button>
                      </div>
                    )}
                  </motion.div>
                )}
        ...
        <div className="fixed bottom-0 inset-x-0 p-6 bg-gradient-to-t from-white dark:from-gray-950 via-white/80 pointer-events-none flex justify-center">
          <div className="w-full max-w-md flex gap-4 pointer-events-auto">
            ...
            <button onClick={() => navigate(1)} disabled={currentIndex === questions.length - 1} className="flex-1 py-4 bg-blue-600 text-white rounded-2xl font-bold shadow-xl shadow-blue-500/20 active:scale-95 disabled:opacity-20 transition-all">下一题 <ChevronRight className="w-4 h-4 inline ml-1"/></button>
          </div>
        </div>
      </div>
    );
